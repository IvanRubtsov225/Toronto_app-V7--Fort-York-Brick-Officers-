...
from src.collectors.reddit_collector_readonly import RedditCollectorReadOnly
from src.analyzers.dinesafe_analyzer import DineSafeAnalyzer
from config.config import SCORING_WEIGHTS

class HiddenGemsFinderReadOnly:
    def __init__(self):
        """Initialize the Hidden Gems Finder with read-only Reddit access"""
        self.reddit_collector = RedditCollectorReadOnly()
    ...
    def load_or_collect_reddit_data(self, use_existing=True, reddit_posts_file=None, reddit_comments_file=None):
        ...
        # Collect new data using read-only collector
        print("Collecting new Reddit data (read-only mode)...")
        posts, comments = self.reddit_collector.collect_all_data(limit_per_subreddit=50)
        ...
        if posts:
            ...
            posts_file = f"reddit_posts_readonly_{timestamp}.csv"
            comments_file = f"reddit_comments_readonly_{timestamp}.csv"
    ...
    def extract_all_mentioned_locations(self):
        ...
        for _, post in self.reddit_data['posts'].iterrows():
            if isinstance(post['locations_mentioned'], str):
                ...
                try:
                    ...
                    locations = [post['locations_mentioned']] if post['locations_mentioned'] else []
            else:
                locations = post['locations_mentioned'] if isinstance(post['locations_mentioned'], list) else []
            
            for location in locations:
                if location and len(str(location).strip()) > 0:
        ...
        # Extract from comments (if any)
        if not self.reddit_data['comments'].empty:
            for _, comment in self.reddit_data['comments'].iterrows():
                if isinstance(comment['locations_mentioned'], str):
                    try:
                        ...
                        locations = [comment['locations_mentioned']] if comment['locations_mentioned'] else []
                else:
    ...
    def export_results(self, filename=None):
        ...
        if filename is None:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            filename = f"toronto_hidden_gems_readonly_{timestamp}.csv"
...
if __name__ == "__main__":
    # Example usage
    finder = HiddenGemsFinderReadOnly()
    ...
    hidden_gems = finder.find_hidden_gems(
        min_dinesafe_score=60,  # Lower threshold for more results
    ...
    if hidden_gems:
        ...
        print("No hidden gems found with current criteria. Try lowering the thresholds.") 